...
export default function ContactInquiriesTab({ isDark = false }: ContactInquiriesTabProps) {
  ...
  const fetchInquiries = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(t("admin_failed_load_affiliate_commissions", "Failed to fetch contact inquiries."));
      }
    } catch {
  ...
  const handleUpdateStatus = async (id: number, status: "Pending" | "Responded" | "Archived") => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(`${t("admin_status_updated_to", "Status updated to")} ${status}`);
      } else {
        showToast(t("admin_failed_update_status", "Failed to update status."), "error");
      }
    } catch {
      showToast(t("admin_network_error_status_update", "Network error during status update."), "error");
    } finally {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm(t("admin_confirm_delete_inquiry", "Are you sure you want to delete this contact inquiry record permanently?"))) return;
    setActionLoading(id);
    try {
      ...
      if (res.ok) {
        ...
        showToast(t("admin_inquiry_deleted_success", "Inquiry deleted successfully!"));
      } else {
        showToast(t("admin_failed_delete_inquiry", "Failed to delete inquiry."), "error");
      }
    } catch {
  ...
  const handleOpenReplyModal = (item: Inquiry) => {
    ...
    setReplyMessage(
      `${t("admin_reply_hello", "Hello")} ${item.name || t("admin_customer_label", "Customer")},\n\n${t("admin_reply_thank_you", "Thank you for reaching out to us regarding")} "${item.subject}".\n\n${t("admin_reply_best_regards", "Best regards,\nSupport Team")}`
    );
  };
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    if (!replyMessage.trim()) {
      showToast(t("admin_enter_reply_message", "Please enter a reply message."), "error");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(t("admin_reply_sent_success", "Reply sent successfully & inquiry marked as Responded!"));
        setReplyingInquiry(null);
      } else {
        const errData = await res.json();
        showToast(errData.error || t("admin_failed_send_reply", "Failed to send reply."), "error");
      }
    } catch {
      showToast(t("admin_network_error_sending_reply", "Network error while sending reply."), "error");
    } finally {
  ...
  return (
    <div className="flex flex-col gap-6 w-full text-left rtl:text-right">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-black tracking-tight">{t("admin_public_contact_inquiries", "Public Contact Form Inquiries")}</h2>
          <p className="text-xs text-slate-500 font-semibold mt-0.5">
            {t("admin_public_contact_inquiries_desc", "Audit and respond to inquiries submitted via the platform public contact page (/contact).")}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className={`p-5 rounded-2xl border flex items-center gap-4 ${isDark ? "bg-slate-950 border-slate-850" : "bg-white border-slate-200/80 shadow-xs"}`}>
          ...
          <div>
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 block">{t("admin_total_submitted", "Total Submitted")}</span>
        ...
        <div className={`p-5 rounded-2xl border flex items-center gap-4 ${isDark ? "bg-slate-950 border-slate-850" : "bg-white border-slate-200/80 shadow-xs"}`}>
          ...
          <div>
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 block">{t("admin_pending_response", "Pending Response")}</span>
        ...
        <div className={`p-5 rounded-2xl border flex items-center gap-4 ${isDark ? "bg-slate-950 border-slate-850" : "bg-white border-slate-200/80 shadow-xs"}`}>
          ...
          <div>
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 block">{t("admin_resolved_responded", "Resolved / Responded")}</span>
      ...
      <div className={`p-4 rounded-2xl border flex flex-col md:flex-row items-center justify-between gap-4 ${isDark ? "bg-slate-950 border-slate-850" : "bg-white border-slate-200/80 shadow-xs"}`}>
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
          {(["All", "Pending", "Responded", "Archived"] as const).map((tab) => (
            ...
            >
              {tab === "All" ? t("admin_all", "All") : tab === "Pending" ? t("pending", "Pending") : tab === "Responded" ? t("admin_responded_label", "Responded") : t("admin_archived_label", "Archived")}
            </button>
          ))}
        </div>
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            type="text"
            placeholder={t("admin_search_inquiries_placeholder", "Search by name, email, message...")}
            value={searchTerm}
      ...
      <div className={`rounded-2xl border overflow-hidden shadow-xs ${isDark ? "bg-slate-950 border-slate-850" : "bg-white border-slate-200/80"}`}>
        {loading ? (
          <div className="p-12 text-center text-slate-400 text-xs font-bold flex flex-col items-center justify-center gap-3">
            ...
            <span>{t("admin_loading_contact_inquiries", "Loading public contact form inquiries...")}</span>
          </div>
        ) : error ? (
          ...
          <div className="p-12 text-center text-slate-400 text-xs font-semibold flex flex-col items-center justify-center gap-3">
            ...
            <div>
              <p className="font-bold text-slate-700 dark:text-slate-200">{t("admin_no_contact_inquiries_found", "No contact inquiries found")}</p>
              <p className="text-[11px] text-slate-400 mt-0.5">{t("admin_no_contact_inquiries_found_desc", "When users submit messages via /contact form, they will appear here automatically.")}</p>
            </div>
          </div>
        ) : (
            ...
            <table className="w-full text-left rtl:text-right border-collapse">
              <thead>
                <tr className={`border-b text-[10px] uppercase font-black tracking-wider ${isDark ? "border-slate-850 text-slate-400 bg-slate-900/50" : "border-slate-150 text-slate-500 bg-slate-50"}`}>
                  <th className="py-3.5 px-4 text-left rtl:text-right">{t("admin_sender_header", "Sender")}</th>
                  <th className="py-3.5 px-4 text-left rtl:text-right">{t("admin_subject_header", "Subject")}</th>
                  <th className="py-3.5 px-4 text-left rtl:text-right">{t("admin_message_snippet_header", "Message Snippet")}</th>
                  <th className="py-3.5 px-4 text-left rtl:text-right">{t("date", "Date")}</th>
                  <th className="py-3.5 px-4 text-left rtl:text-right">{t("admin_status_selector_header", "Status Selector")}</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-850 text-xs">
                {filteredInquiries.map((item) => (
                  ...
                  >
                    <td className="py-4 px-4 whitespace-nowrap text-left rtl:text-right">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="font-black text-slate-800 dark:text-slate-100 truncate">{item.name || t("admin_anonymous_sender", "Anonymous Sender")}</span>
                    ...
                    <td className="py-4 px-4 max-w-xs text-left rtl:text-right">
                      <p className="text-slate-500 dark:text-slate-400 text-xs truncate max-w-[220px]">
                        {item.message}
                      </p>
                    </td>

                    <td className="py-4 px-4 whitespace-nowrap text-slate-400 text-[11px] font-semibold text-left rtl:text-right">
                    ...
                    <td className="py-4 px-4 whitespace-nowrap text-left rtl:text-right">
                      ...
                      >
                        ...
                        <option value="Responded">{t("admin_responded_label", "Responded")}</option>
                        <option value="Archived">{t("admin_archived_label", "Archived")}</option>
                      </select>
                    </td>
                    ...
                    <td className="py-4 px-4 whitespace-nowrap text-right rtl:text-left">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        >
                          {t("admin_view_message_btn", "View Message")}
                        </button>

                        <button
                          onClick={() => handleOpenReplyModal(item)}
                          className="px-2.5 py-1.5 rounded-lg bg-teal-700 hover:bg-teal-800 text-white text-[11px] font-bold transition flex items-center gap-1 cursor-pointer shadow-xs border-0"
                        >
                          <FiMail className="w-3 h-3 text-white" /> {t("admin_reply_btn", "Reply")}
                        </button>
                        ...
                        <button
                          ...
                          title={t("admin_delete_inquiry_title", "Delete inquiry")}
                        >
      ...
      {mounted && selectedInquiry && createPortal(
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-[2px] z-[99999] flex items-center justify-center p-4 animate-fadeIn">
          <div className={`w-full max-w-lg rounded-2xl shadow-2xl border p-6 flex flex-col gap-4 animate-scaleUp text-left rtl:text-right relative ${
            ...
            <div className="flex items-start justify-between border-b pb-4 border-slate-100 dark:border-slate-850">
              <div className="text-left rtl:text-right">
                <span className="text-[10px] font-black uppercase tracking-wider text-teal-600">{t("admin_public_contact_inquiry_badge", "Public Contact Form Inquiry")}</span>
                <h3 className="text-base font-black text-slate-900 dark:text-white mt-0.5">{selectedInquiry.subject}</h3>
              </div>
              <button
                onClick={() => setSelectedInquiry(null)}
                className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-900 transition cursor-pointer border-0 bg-transparent"
              >
                <FiX className="w-5 h-5" />
              </button>
            </div>

            <div className="grid grid-cols-2 gap-3 text-xs bg-slate-50 dark:bg-slate-900 p-3.5 rounded-xl border border-slate-150 dark:border-slate-800 text-left rtl:text-right">
              <div>
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t("admin_sender_name_label", "Sender Name")}</span>
                <span className="font-bold text-slate-800 dark:text-slate-200">{selectedInquiry.name || t("admin_anonymous_sender", "Anonymous Sender")}</span>
              </div>
              <div>
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t("email_address", "Email Address")}</span>
                <span className="font-bold text-slate-800 dark:text-slate-200 break-all">{selectedInquiry.email}</span>
              </div>
              <div>
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t("admin_submitted_at_label", "Submitted At")}</span>
                <span className="font-semibold text-slate-500">{new Date(selectedInquiry.created_at).toLocaleString()}</span>
              </div>
              <div>
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t("admin_current_status_label", "Current Status")}</span>
                <select
            ...
            <div className="text-left rtl:text-right">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block mb-1.5">{t("admin_inquiry_message_body_label", "Inquiry Message Body")}</span>
            ...
            <div className="flex items-center justify-between gap-3 border-t pt-4 border-slate-100 dark:border-slate-850 mt-2">
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-bold transition cursor-pointer border-0"
                >
                  {t("admin_mark_responded_btn", "Mark Responded")}
                </button>
                <button
                  onClick={() => handleUpdateStatus(selectedInquiry.id, "Archived")}
                  className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition cursor-pointer border-0"
                >
                  {t("admin_archive_btn", "Archive")}
                </button>
              </div>
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-teal-700 hover:bg-teal-800 text-white text-xs font-bold transition flex items-center gap-1.5 cursor-pointer shadow-xs border-0"
              >
                <FiMail className="w-3.5 h-3.5" /> {t("admin_reply_to_inquiry_btn", "Reply to Inquiry")}
              </button>
            </div>
          </div>
        </div>,
      ...
      {mounted && replyingInquiry && createPortal(
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-[2px] z-[99999] flex items-center justify-center p-4 animate-fadeIn">
          <form
            ...
            className={`w-full max-w-lg rounded-2xl shadow-2xl border p-6 flex flex-col gap-4 animate-scaleUp text-left rtl:text-right relative ${
              isDark ? "bg-slate-950 border-slate-800 text-white" : "bg-white border-slate-200 text-slate-800"
            }`}
          >
            <div className="flex items-start justify-between border-b pb-4 border-slate-100 dark:border-slate-850">
              <div className="text-left rtl:text-right">
                <span className="text-[10px] font-black uppercase tracking-wider text-teal-600">{t("admin_send_response_customer_header", "Send Response to Customer")}</span>
                <h3 className="text-base font-black text-slate-900 dark:text-white mt-0.5">
                  {t("admin_reply_to_label", "Reply to ")}{replyingInquiry.name || replyingInquiry.email}
                </h3>
              </div>
              <button
                type="button"
            ...
            <div className="flex flex-col gap-3 text-left rtl:text-right">
              <div>
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 block mb-1">{t("admin_to_email_address_label", "To Email Address")}</label>
                <input
                  type="text"
                  disabled
                  value={replyingInquiry.email}
                  className="w-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 opacity-80 cursor-not-allowed text-left rtl:text-right"
                />
              </div>

              <div>
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 block mb-1">{t("admin_subject_line_label", "Subject Line")}</label>
                <input
                  type="text"
                  required
                  value={replySubject}
                  onChange={(e) => setReplySubject(e.target.value)}
                  className={`w-full border rounded-xl px-3.5 py-2 text-xs font-bold transition focus:outline-none text-left rtl:text-right ${
                    isDark
              ...
              <div>
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 block mb-1">{t("admin_message_body_label", "Message Body")}</label>
                <textarea
                  rows={6}
                  required
                  value={replyMessage}
                  onChange={(e) => setReplyMessage(e.target.value)}
                  className={`w-full border rounded-xl p-3.5 text-xs font-medium transition focus:outline-none leading-relaxed text-left rtl:text-right ${
                    isDark
                      ? "bg-slate-900 border-slate-800 text-white focus:border-teal-500"
                      : "bg-white border-slate-250 text-slate-850 focus:border-teal-500"
                  }`}
            ...
            <div className="flex items-center justify-between gap-3 border-t pt-4 border-slate-100 dark:border-slate-850 mt-2">
              ...
              >
                <FiExternalLink className="w-3.5 h-3.5" /> {t("admin_open_desktop_email_app", "Open in Desktop Email App")}
              </a>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-6 py-2 rounded-xl bg-teal-700 hover:bg-teal-800 text-white text-xs font-bold transition flex items-center gap-2 cursor-pointer shadow-xs disabled:opacity-50 border-0"
                >
                  {sendingReply ? (
                    <>
                      <FiLoader className="w-3.5 h-3.5 animate-spin" />
                      <span>{t("admin_sending_reply", "Sending Reply...")}</span>
                    </>
                  ) : (
                    <>
                      <FiSend className="w-3.5 h-3.5 text-white" />
                      <span>{t("admin_send_email_reply", "Send Email Reply")}</span>
                    </>
                  )}
                </button>
              </div>
